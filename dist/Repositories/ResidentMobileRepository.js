...
const addMobileResident = (payload) =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
      ...
      const sql_insert_user = yield con.Insert(
        `INSERT user SET
        email=@email,
      ...
      if (sql_insert_user.insertedId > 0) {
        ...
        const resident_payload = Object.assign(Object.assign({}, payload), {
          user_pk: sql_insert_user.insertedId,
          birth_date: useDateParser_1.parseInvalidDateToDefault(
            payload.birth_date
          ),
        });
        const sql_add_resident = yield con.Insert(
...
const updateMobileResident = (payload, user_pk) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const con = yield DatabaseConfig_1.DatabaseConnection();
    try {
      yield con.BeginTransaction();
      ...
      console.log(payload);
      const update_user = yield con.Insert(
        `UPDATE user SET
      email=@email,
      password=AES_ENCRYPT(@email,@email),
      full_name=@full_name
      ...
      const sql_edit_resident = yield con.Modify(
        `UPDATE resident SET
        user_pk=@user_pk,
        pic=@pic,              
        first_name=@first_name,       
...
const updatepassword = (email, password, currentpassword) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const con = yield DatabaseConfig_1.DatabaseConnection();
    try {
      const sql_edit_resident = yield con.Modify(
